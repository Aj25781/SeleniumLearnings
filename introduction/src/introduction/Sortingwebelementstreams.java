...
public class Sortingwebelementstreams {
	...
	public static void main(String[] args) {
		...
		/*
		 * List<String>actualList=elements.stream().map(s->s.getText()).collect(
		 * Collectors.toList());
		 * 
		 * List<String>listAfterSort=actualList.stream().sorted().collect(Collectors.
		 * toList());
		 * 
		...
		do {
			...
			priceOfVegiige = elements.stream().filter(s -> s.getText().contains("Potato")).map(s -> getVaggies(s))
